...
export const ProjectionScreen = ({ eventId }: ProjectionScreenProps) => {
  ...
  return (
    <div className="h-screen w-screen text-white overflow-hidden relative">
      ...
      {/* Overlay oscuro sutil para mejorar contraste global (especialmente en fondos claros) */}
          ...
          >
            {items[currentIndex]?.type === 'message' && (
              <div className="relative max-w-4xl w-full">
                {/* Icono de mensaje flotante (estilo premium) */}
                <div className="absolute -top-12 left-1/2 -translate-x-1/2 w-24 h-24 bg-white rounded-full flex items-center justify-center shadow-2xl z-20 border-4 border-white">
      ...
      <div className="absolute top-12 left-0 w-full text-center z-20 pointer-events-none px-12">
        <p className="text-3xl font-black uppercase tracking-[0.5em] text-white drop-shadow-[0_2px_10px_rgba(0,0,0,0.5)]">
          {eventName}
        </p>
      </div>
    </div>
